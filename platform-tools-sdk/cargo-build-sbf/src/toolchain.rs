use {
    ...
    regex::Regex,
    trezoa_file_download::download_file,
    std::{
        env,
        fs::{self, File},
        io::BufReader,
...
fn link_trezoa_toolchain(config: &Config, requested_toolchain_version: &str) {
    let toolchain_path = config
        .sbf_sdk
        .join("dependencies")
        .join("platform-tools")
...
pub(crate) fn install_tools(
    ...
    if config.no_rustup_override {
        ...
        link_trezoa_toolchain(config, &platform_tools_version);
