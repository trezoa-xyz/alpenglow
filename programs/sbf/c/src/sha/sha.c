...
extern uint64_t entrypoint(const uint8_t *input) {
  ...
  // SHA256
  {
    ...
    trz_sha256(bytes, TRZ_ARRAY_SIZE(bytes), result);

    trz_assert(0 == trz_memcmp(result, expected, SHA256_RESULT_LENGTH));
  }
  ...
  // Keccak
  {
    ...
    trz_keccak256(bytes, TRZ_ARRAY_SIZE(bytes), result);

    trz_assert(0 == trz_memcmp(result, expected, KECCAK_RESULT_LENGTH));
  }
  ...
  // Blake3
  {
    ...
    trz_blake3(bytes, TRZ_ARRAY_SIZE(bytes), result);

    trz_assert(0 == trz_memcmp(result, expected, BLAKE3_RESULT_LENGTH));
  }
