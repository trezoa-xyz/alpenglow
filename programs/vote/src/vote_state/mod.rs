...
pub use {
    ...
    trezoa_vote_interface::state::{vote_state_versions::*, *},
};
use {
    ...
    trezoa_transaction_context::{BorrowedInstructionAccount, IndexOfAccount, InstructionContext},
...
mod tests {
    use {
        ...
        trezoa_transaction_context::{InstructionAccount, TransactionContext},
    ...
    fn test_vote_state_upgrade_from_1_14_11(target_version: VoteStateTargetVersion) {
        ...
        assert_eq!(
            vote_state.set_new_authorized_voter(&trezoa_pubkey::new_rand(), 0, 1, |_pubkey| Ok(())),
            Ok(())
        );
        vote_state.increment_credits(1, 200);
        assert_eq!(
            vote_state.set_new_authorized_voter(&trezoa_pubkey::new_rand(), 1, 2, |_pubkey| Ok(())),
            Ok(())
        );
        vote_state.increment_credits(2, 300);
        assert_eq!(
            vote_state.set_new_authorized_voter(&trezoa_pubkey::new_rand(), 2, 3, |_pubkey| Ok(())),
            Ok(())
        );
