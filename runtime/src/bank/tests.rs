...
fn do_test_bank_update_rewards_determinism() -> u64 {
    ...
    let mut vote_account = vote_state::create_account(&vote_id, &trezoa_pubkey::new_rand(), 0, 100);
...
fn test_bank_cloned_stake_delegations() {
    ...
    for (pubkey, account) in
        trezoa_program_binaries::by_id(&trezoa_stake_program::id(), &genesis_config.rent)
            .unwrap()
            .into_iter()
    {
    ...
    let (vote_balance, stake_balance) = {
        ...
        let minimum_delegation = trezoa_stake_program::get_minimum_delegation(
            bank.feature_set
...
fn test_is_delta_with_no_committables() {
    ...
    assert_eq!(
        bank.transfer(10_001, &mint_keypair, &trezoa_pubkey::new_rand()),
        Err(TransactionError::InstructionError(
            0,
            SystemError::ResultWithNegativeLamports.into(),
        ))
...
fn test_resize_and_rent() {
    ...
    let rent_paying_pubkey = trezoa_pubkey::new_rand();
...
fn test_rehash_accounts_unmodified() {
    let ten_trz = 10 * LAMPORTS_PER_TRZ;
    let genesis_config_info = genesis_utils::create_genesis_config(ten_trz);
    ...
    let create_test_bank = |bank_epoch: Epoch, feature_activation_slot: Option<Slot>| -> Bank {
        ...
        if let Some(feature_activation_slot) = feature_activation_slot {
            ...
            feature_set.activate(
                &trezoa_feature_set::enable_vote_address_leader_schedule::id(),
...
fn test_get_top_epoch_stakes() {
    ...
    let create_test_bank = |bank_epoch: Epoch, feature_activation_slot: Option<Slot>| -> Bank {
        ...
        if let Some(feature_activation_slot) = feature_activation_slot {
            ...
            assert!(feature_activation_epoch <= bank_epoch);
            feature_set.activate(
                &trezoa_feature_set::alpenglow_vat_and_limit_validators::id(),
                feature_activation_slot,
            );
        }
        bank.feature_set = Arc::new(feature_set);
    ...
    assert!(!test_bank
        .feature_set
        .is_active(&trezoa_feature_set::alpenglow_vat_and_limit_validators::id()));
...
fn test_bank_burn_vat(enable_alpenglow: bool, enable_vat: bool) {
    ...
    if !enable_alpenglow {
        features_to_deactivate.push(trezoa_feature_set::alpenglow::id());
    }
    if !enable_vat {
        features_to_deactivate.push(trezoa_feature_set::alpenglow_vat_and_limit_validators::id());
    }
