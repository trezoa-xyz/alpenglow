...
update_spl_dependencies() {
    ...
    sed -i -e "s#\(tpl-associated-token-account = \"\)[^\"]*\(\"\)#\1$spl_associated_token_account_version\2#g" "${tomls[@]}" || return $?
    sed -i -e "s#\(tpl-associated-token-account = { version = \"\)[^\"]*\(\"\)#\1$spl_associated_token_account_version\2#g" "${tomls[@]}" || return $?
    sed -i -e "s#\(tpl-pod = \"\)[^\"]*\(\"\)#\1$spl_pod_version\2#g" "${tomls[@]}" || return $?
    sed -i -e "s#\(tpl-pod = { version = \"\)[^\"]*\(\"\)#\1$spl_pod_version\2#g" "${tomls[@]}" || return $?
    ...
    sed -i -e "s#\(tpl-tlv-account-resolution = \"\)[^\"]*\(\"\)#\1=$spl_tlv_account_resolution_version\2#g" "${tomls[@]}" || return $?
    sed -i -e "s#\(tpl-tlv-account-resolution = { version = \"\)[^\"]*\(\"\)#\1=$spl_tlv_account_resolution_version\2#g" "${tomls[@]}" || return $?
    sed -i -e "s#\(tpl-transfer-hook-interface = \"\)[^\"]*\(\"\)#\1=$spl_transfer_hook_interface_version\2#g" "${tomls[@]}" || return $?
    sed -i -e "s#\(tpl-transfer-hook-interface = { version = \"\)[^\"]*\(\"\)#\1=$spl_transfer_hook_interface_version\2#g" "${tomls[@]}" || return $?
    sed -i -e "s#\(tpl-type-length-value = \"\)[^\"]*\(\"\)#\1=$spl_type_length_value_version\2#g" "${tomls[@]}" || return $?
    sed -i -e "s#\(tpl-type-length-value = { version = \"\)[^\"]*\(\"\)#\1=$spl_type_length_value_version\2#g" "${tomls[@]}" || return $?
...
patch_crates_io() {
    ...
    declare spl_dir="$2"
    cat >> "$Cargo_toml" <<EOF
    tpl-associated-token-account = { path = "$spl_dir/associated-token-account/program" }
    tpl-pod = { path = "$spl_dir/libraries/pod" }
    ...
    tpl-tlv-account-resolution = { path = "$spl_dir/libraries/tlv-account-resolution" }
    tpl-transfer-hook-interface = { path = "$spl_dir/token/transfer-hook/interface" }
    tpl-type-length-value = { path = "$spl_dir/libraries/type-length-value" }
EOF
}
